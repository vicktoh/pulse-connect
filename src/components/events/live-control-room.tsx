...
import { useReformCommitments, useTrackerVotes } from "@/lib/events/reform-tracker"
...
export function LiveControlRoom() {
  ...
  const trackerVotes = useTrackerVotes(sessionId)
      ...
      <div className={styles.controlGrid}>
        ...
        {!session ? <section className={styles.noSession}><Radio /><h1>Create the first live session</h1><p>The QR, games, tracker and audience tools will appear here.</p></section> : (
          <section className={styles.dashboard}>
            <div className={styles.sessionTopline}><div><span><i /> {session.status.toUpperCase()} SESSION</span><h1>{session.name}</h1><p>{participantCount} attendees joined · {published.length} Reform Signals published</p></div><div className={styles.sessionActions}><Link href={`/live?session=${session.code}`} target="_blank"><MonitorUp /> Open projector <ArrowUpRight /></Link><button type="button" onClick={() => void closeSession()}><Power /> End session</button></div></div>
            ...
            <div className={styles.studioGrid}>
              ...
              <section className={styles.previewPanel}><PanelHeading icon={<MonitorUp />} title="PREVIEW" detail={cue.kind === "tracker" ? "Next on projector + joined devices" : "Not visible to audience"} /><BroadcastPreview cue={cue} commitments={commitments} /><button className={styles.takeLive} onClick={() => void takeLive()} disabled={!producer.isOwner || busy}><Send /> {busy ? "Sending…" : "Take live"}</button>{!producer.isOwner && <small className={styles.controlHint}>Claim live control to broadcast this preview.</small>}</section>

              <section className={styles.onAirPanel}><PanelHeading icon={<Radio />} title="ON AIR NOW" detail={session.activeExperience === "tracker" ? `${trackerVotes.length} tracker votes` : `${responses.length} responses`} /><div className={styles.onAirTitle}><i /><strong>{session.activeExperience === "prediction" ? "Promise or Progress" : session.activeExperience}</strong></div>{activePrompt ? <LiveResponseView prompt={activePrompt} pollCounts={pollCounts} words={words} total={responses.filter((item) => item.visible).length} /> : <div className={styles.panelEmpty}>{session.activeExperience === "tracker" ? "The projector and every joined device are showing the interactive Reform Tracker." : `The venue screen is showing ${session.activeExperience}.`}</div>}{activePrompt && <button className={styles.endInteraction} onClick={() => void endInteraction()} disabled={!producer.isOwner || busy}><Check /> Close interaction & return phones</button>}</section>
